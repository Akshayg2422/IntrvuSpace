...
import { Modal, Image, Button, showToast } from '@Components';
...
function DropzoneFilePicker({ onSelect, isOpen, size, title, onSubmitClick, isUploadModalOpen = false, isDownloadTemplate = false, onTemplateClick, bulkButtonSize="lg", bulkButtonStyle, outline }: UploadProps) {
    ...
    const selectHandler = (e: any) => {
        ...
        if (check != null) {
            ...
            reader.onload = () => {
                let encoded = reader && reader.result && reader.result.toString().replace('data:', '').replace(/^.+,/, '');
                if (onSelect) {
                    onSelect(encoded);
                    setFileData(encoded)
                ...
                >
                    ...
                    {!fileData && (
                        ...
                        >
                            ...
                            <button className="dz-button" type="button">{'Drop file here to upload'}</button>
                        </div>
